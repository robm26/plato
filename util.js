...
const randomChoiceSeeded = (arr, seed) => {

    const seededBigInt = parseInt(md5(seed.toString()), 16) % 100000;
    const seededChoice = seededBigInt % arr.length;

    return arr[seededChoice];
}

const payloadData = (format, kb) => {
    let payload;
    ...
    if(format && format.toLowerCase() === 'json') {
        ...
        obj['name'] = 'Sam';
        obj['rating'] = 345;
        obj['entitlements'] = ['product1','product2','product3'];

        payload = marshall(obj);
...
export {randomString, md5, payloadData, randomChoiceSeeded};
